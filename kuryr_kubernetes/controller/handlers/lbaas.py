...
class LBaaSSpecHandler(k8s_base.ResourceEventHandler):
    ...
    def _should_ignore(self, service):
        return (not(self._has_selector(service)) or
                ...
                not(self._is_supported_type(service))or
                self._should_ignorename(service))
    '''
    def _should_ignore(self, service):
        LOG.error("Enter _should_ignore")
        LOG.error("has_selector: %s\n", (self._has_selector(service)))
        LOG.error("has_clusterip: %s\n", (self._has_clusterip(service)))
        LOG.error("is_supported_type: %s\n", (self._is_supported_type(service)))
        LOG.error("is_ignorename: %s\n", self._should_ignorename(service))
        return (not(self._has_selector(service)) or
                not(self._has_clusterip(service)) or
                not(self._is_supported_type(service)) or
                self._should_ignorename(service))
    '''
    def _should_ignorename(self, service):
        '''LOG.error("service['metadata'].get('name') : %s\n", service['metadata'].get('name'))
        LOG.error("service['metadata']['name'] : %s\n", service['metadata']['name'])'''
        return service['metadata'].get('name') == 'kube-dns'
...
class LoadBalancerHandler(k8s_base.ResourceEventHandler):
    ...
    def on_present(self, endpoints):
        ...
        if self._should_ignore(endpoints, lbaas_spec):
            ...
            elif not self._svc_handler_annotations_updated(endpoints,
                                                         lbaas_spec):
                reason = 'Not updated'
    ...
    def _should_ignore(self, endpoints, lbaas_spec):
        ...
        return not(lbaas_spec and
                   ...
                   self._svc_handler_annotations_updated(endpoints,
                                                         lbaas_spec) and
                   self._is_validendpoints(endpoints))

    def _is_validendpoints(self, endpoints):
        return endpoints['metadata'].get('name') != 'kube-controller-manager' and endpoints['metadata'].get('name') != 'kube-scheduler'
